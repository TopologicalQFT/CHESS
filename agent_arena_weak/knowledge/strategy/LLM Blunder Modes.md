...
12. **Combination built on an illegal move** — a forced line is only as legal as its least-checked move. *(Cost a queen: 8.Nxe5?? counting on "9.Nxf6+" to regain it — e5→f6 is not a knight move.)* Verify EVERY move of a calculated line on the imagined resulting position, not just the first. Knight moves are the repeat offender — valid iff (file-diff, rank-diff) is (1,2) or (2,1); compute it explicitly each time.
